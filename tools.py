...
def calc_mixing_length(N,L, bndry_bot, bndry_top):
    ...
    rel_height = np.linspace(0,L,N)/ np.full(N,L)
    if bndry_bot == "dirichlet" and bndry_top == "dirichlet":
        mixing_length = np.full(N,0.14) - np.full(N,0.08) * (np.ones(N)-2*rel_height)**2 - np.full(N,0.06) * (np.ones(N)-2*rel_height)**4
    else:
        mixing_length = np.full(N,0.14) - np.full(N,0.08) * (np.ones(N)-rel_height)**2 - np.full(N,0.06) * (np.ones(N)-rel_height)**4
